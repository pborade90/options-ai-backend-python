...
def get_yearly_analysis(stock_symbol):
    ...
    yearly_summary = data.groupby('Year').agg({
        'Open': 'first',    # First recorded price of the year
        'Close': 'last',    # Last recorded price of the year
        'High': 'max',      # Highest price in that year
        'Low': 'min',       # Lowest price in that year
        'Volume': 'sum'     # Total trading volume in that year
    }).reset_index()
...
def fetch_stock(stock_symbol: str):
    ...
    stock_data = [
        {
            "id": generate_unique_id(),  # ✅ Ensure a new unique ID is generated
            "date": str(row["Date"].date()),  # Convert timestamp to date
            "open": float(row["Open"]),
            "close": float(row["Close"]),
            "high": float(row["High"]),
            "low": float(row["Low"]),
    ...
    return {
        "stock_symbol": stock_symbol,
        "data": stock_data  # ✅ Now includes unique IDs
    }
